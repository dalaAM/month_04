...
- 一个重量级的 Python Web框架，Django 配备了常用的大部分组件
    ...
    1. 原生HTML模板系统(前端，展示数据)(大型项目，有前端框架)
    
    1. *视图 view --（匹配路由，处理响应）
    
    1. Model模型（Models.py),数据库连接和ORM（利用面向对象的方式替代sql语句操作数据库）数据库管理 --(从数据库中拿数据，对数据库做增删改查)
    ...
    1. 数据库后台管理系统admin
       ...
       当浏览器将请求发送给Django服务器，Django的主路由（c）接收请求将请求解析后，分别将不同请求交由对应的view方法去处理，view方法接收相应的参数  
    
       Model 访问数据库，将增删改查的结果返回，
    
       HttpResponse（request，'list.html',模型数据）（对数据的渲染和models）
    
       ![](/home/tarena/下载/cMTV.png)
    
       MVC架构
    
       浏览器将请求发送给服务器，由controller层接收请求，同时分配任务
    
       Model层负责处理具体的业务逻辑和访问数据库;同时将数据返回给controller
    
       View层 负责界面，同时将数据返回给con'troller
    
       ![](/home/tarena/下载/month_04/day01/images/mvc1.png)
...
- sudo  ps aux|grep 'runserver'  查询进程名，找到进程编号
- kill -9   进程编号 ：杀死进程
2.`方案2`
- sudo lsof -i:8000  查询进程所占用的端口号，找到进程编号
- kill -9   进程编号 ：杀死进程
...
4. `INSTALLED_APPS` **day03涉及**
   
    - 指定当前项目中安装的应用列表
5. `MIDDLEWARE`  **day07涉及**
...
| slug（不常用） | 匹配任意由 ASCII 字母或数字以及连字符和下划线组成的短标签 | "detail/\<slug:sl>" 匹配 /detail/this-is-django     |
| path（不常用） | 匹配非空字段，包括路径分隔符 `'/'`                        | "v1/users/\<path:ph>"匹配 /v1/goods/a/b/c           |
...
- HTTP1.1 请求详述
    ...
    | 4 | PUT | 从客户端向服务器传送的数据取代指定的文档的内容。（修改内容） |
    | 5 | DELETE | 请求服务器删除指定的页面。（model）不是清空数据库中信息，而是增加一个标记 |
...
```python
*HTTP/1.0(服务器协议) 200（状态码） OK（状态）                        -> 起始行
