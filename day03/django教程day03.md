...
## 静态文件
1. 什么是静态文件
    - 不能与服务器端做动态交互的文件都是静态文件（不能动态修改的 如：404页面）
...
2. 创建 和 配置数据库
    1. 创建数据库
        ...
        ```sql
        create database mywebdb default charset utf8 collate utf8_general_ci（不区分大小写）;
        ```
    2. 数据库的配置
        - sqlite 数据库配置
            ```python
...
- 模型类需继承自`django.db.models.Model`（每个模型的子类都对应的是数据库中的一个表）
...
5. DecimalField()
    ...
    - 在数据库中:使用小数
    - 参数:
        - max_digits: 位数总数，包括小数点后的位数。 该值必须大于等于decimal_places.（有效数字）
...
6. FloatField()
    - 数据库类型:double
    - 编程语言中和数据库中都使用小数表示值（表示的近似值）浮点数不做相等的比较，在底层存储时是做近似存储，不是精确存储
7. EmailField()存储电子邮件的
    - 数据库类型:varchar
    - 编程语言和数据库中使用字符串
8. IntegerField() 存储整数的
    - 数据库类型:int
    - 编程语言和数据库中使用整数
9. URLField() 存储url的
    - 数据库类型:varchar(200)
...
2. blank
    - 设置为True时，字段可以为空。设置为False时，字段是必须填写的。代表的是空白
3. null
    - 如果设置为True,表示该列值允许为空。
